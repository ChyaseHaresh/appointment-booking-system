...
<body>
    ...
    <div class="row">
        <nav class="navbar navbar-expand-lg py-0">
            <div class="container-fluid" style="margin-left: 150px ;">
                <a class="navbar-brand m-0 display-5 text-uppercase text-primary" href="#"
                    style="font-size: 2.5em;"><i class="fa-sharp fa-solid fa-globe"></i> Global Study</a>
                ...
                <div class="collapse navbar-collapse" id="navbarNavDarkDropdown">
                    <ul class="nav mx-auto navbar-nav">
                        ...
                        <div>
                            <a href="{{ url('appointment') }}" class="btn btn-primary mt-2 px-4 d-none d-lg-block">Book
                                an appointment</a>
                        </div>
                </div>
        </nav>
    ...
    <!-- ----Footer-- -->
    <div class="container-fluid"style="background-color: #a3c2f7;">
        <div class="container mt-5">
            <footer class="py-5">
                <div class="row">
                    ...
                    <div class="col-6 col-md-4 mb-3">
                        <ul class="nav flex-column">
                            <li class="nav-item mb-2"><a href="#" class="nav-link p-0 ">Mini IELTS Mock Test</a>
                            </li>
    ...
    @php
        ...
        $slots = new Carbon('10:00');
        $time_slots=array();
        for ($i=0; $i <= 7; $i++) {
            array_push($time_slots, $slots->addHour()->isoFormat('H:mm'));
        }
            ...
            $("#select_counselor").change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // $('#available_slots').find('button').remove();
                        $("#available_slots").html(response['states'])
                        // document.write(response['states'])

                //         if (response['states'].length > 0) {

                //             $.each(response['states'], function(key, value) {
                //                 key=0
                //                 jQuery("#available_slots").html(

                //                     // "<option value='" + value[
                //                     //     'id'] +
                //                     // "'>" + value['name'] + "</option>"

                //                     // if (value['time'] - > isoFormat('H:mm') ===$dato - > isoFormat('H:mm')) {

                //                         // '<option value="'.$list->id.'">'.$list->state.'</option>';

                //                         `<button class="btn btn-primary times" type="button" onclick="myFunction(this)"
                // data-bs-toggle="modal" data-bs-target="#exampleModal">` + `{{ $time_slots[0] }}` + `</button>`
                //                     // } else {
